  ...
  for (size_t i = 0; i < N; ++i) {
    for (size_t j = 0; j < M; ++j) {
      for (size_t k = 0; k < L; ++k) {
        char strelem[20];
        snprintf(strelem, 20, "%e ", m[i * M * L + j * L + k]);
        msg += strelem;
      }
      msg += '\n';
    }
...
float PMFModel::train(const Dataset& train_set) {
  ...
  float last_msre = Model::test(train_set);
  LOG(INFO) << "Init: Loss = " << last_loss << " MSRE = " << last_msre;
  ...
  for (uint32_t iter = 1; iter <= max_iters_; ++iter) {
    compute_loss_grad(norm_data, C, N, M, D_, Y_, V_, W_, HY_,
    ...
    last_loss = compute_loss(
        norm_data, C, N, M, D_, Y_, V_, W_, HY_, lY_, lV_, lW_);
    last_msre = Model::test(train_set);
    LOG(INFO) << "Iter. " << iter << ": Loss = "
              << last_loss << " MSRE = " << last_msre;
  }
  delete [] dY;
  delete [] dV;
  delete [] dW;
...
std::string PMFModel::info() const {
  ...
  char buff[buff_size];
  std::string msg;
  snprintf(buff, buff_size, "Factors = %u\n", D_);
  msg += buff;
  snprintf(buff, buff_size, "Max. Iters = %u\n", max_iters_);
  msg += buff;
  snprintf(buff, buff_size, "Learning rate = %f\n", learning_rate_);
  msg += buff;
  snprintf(buff, buff_size, "Momentum = %f\n", momentum_);
  msg += buff;
  snprintf(buff, buff_size, "lY = %f\n", lY_);
  msg += buff;
  snprintf(buff, buff_size, "lV = %f\n", lV_);
  msg += buff;
  snprintf(buff, buff_size, "lW = %f\n", lW_);
  msg += data_.info(0);
  return msg;
}
